...
assert("latte uses 1 cup", latte["Cups"] === 1);
assert("latte does not use matcha", latte["Matcha Powder"] == null);

const americano = usedMap("americano-long-black", 1);
assert("americano uses beans+milk+sugar+cup", Boolean(americano["Coffee Beans"] && americano["Milk"] && americano["Sugar"] && americano["Cups"]));
...
assert("matcha uses 1 cup", matchaLatte["Cups"] === 1);
...
assert("non-coffee uses milk+sugar+cup", strawberry["Milk"] === roundQty(1000 / 7.5) && strawberry["Sugar"] === 10 && strawberry["Cups"] === 1);
assert("non-coffee does not use beans", strawberry["Coffee Beans"] == null);

const fresh = usedMap("watermelon-whisper", 1);
assert("fresh drink uses milk+sugar+cup", Boolean(fresh["Milk"] && fresh["Sugar"] && fresh["Cups"]));
...
assert("5 lattes = 5 cups", fiveLattes["Cups"] === 5);
...
assert("mixed cart cups remaining", after["Cups"] === expectedCups);
